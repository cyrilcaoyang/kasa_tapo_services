...
# Media root for snapshots + recordings. Must match KASA_TAPO_MEDIA_ROOT
# in kasa-tapo-services.service. Override there if you want a different
# location.
...
`ProtectSystem=strict` plus `ProtectHome=true` in the unit file mean any
write target outside `ReadWritePaths=` will EROFS at runtime. If you set
a per-camera `media:` path that is **not** under
`/var/lib/kasa-tapo-media`, append it to `ReadWritePaths=` in the unit
and reload (`sudo systemctl daemon-reload`). For paths under `/home`
you must also drop `ProtectHome=true`.
...
* Tail logs: `journalctl -u kasa-tapo-services.service -f`
* Restart after editing `devices.yaml`: `sudo systemctl restart kasa-tapo-services.service ac-go2rtc.service`
* Re-render go2rtc config without bouncing the gateway:
  `sudo -u ac-lab bash -lc 'cd /opt/kasa-tapo-services && uv run kasa-tapo-bootstrap-go2rtc --output /etc/go2rtc/go2rtc.yaml' && sudo systemctl restart ac-go2rtc.service`
...
5. Verify the RTSP paths from the dashboard host:
   ...
   ```
6. Add an entry to `/opt/kasa-tapo-services/devices.yaml` (see `../devices.yaml.example` for the shape).
